...
static void* _Data_handle(void* sock_fd);   //Only can be seen in the file
...
int main()
{
    ...
    while(1)
    {
       ...
       if(pthread_create(&thread_id,NULL,&_Data_handle,&sockfd) == -1)
        {
...
static void* _Data_handle(void * sock_fd)
{
    int fd = *((int *)sock_fd);
    int i_recvBytes;
